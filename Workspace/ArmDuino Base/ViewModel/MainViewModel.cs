    ...
    class MainViewModel
    {
        ...
        public void KinectMapper()
        {
            ...
            if (KinectHandler.Sensor != null && KinectHandler.Sensor.SkeletonStream.IsEnabled && currentSkeleton != null)
            {
                ...
                double horizontal2angle = KinectHandler.RightJointsAnglePonderated(currentSkeleton.Joints[JointType.ShoulderRight], currentSkeleton.Joints[JointType.ElbowRight], currentSkeleton.Joints[JointType.WristRight], currentSkeleton, 1, 180);
                double horizontal3angle = KinectHandler.RightJointsAnglePonderated(currentSkeleton.Joints[JointType.ElbowRight], currentSkeleton.Joints[JointType.WristRight], currentSkeleton.Joints[JointType.HandRight], currentSkeleton, 1, 180);
                Arm.Horizontal1Ang = (int)horizontal1angle;
                Arm.Horizontal2Ang = (int)horizontal2angle;
                Arm.Horizontal3Ang = (int)horizontal3angle;
                if (KinectHandler.Grip) Arm.Pinza = 30;
                else Arm.Pinza = 0;
